...
struct ThemeTokensTests {
    ...
    @Test func namedPalettesAreDistinctAndFillEveryToken() {
        ...
        for family in ThemeFamily.allCases {
            ...
            for tokens in [light, dark] {
                ...
                #expect(tokens.h1.cgColor.alpha > 0)
                #expect(tokens.h2.cgColor.alpha > 0)
                #expect(tokens.h3.cgColor.alpha > 0)
                #expect(tokens.h4.cgColor.alpha > 0)
                #expect(tokens.h5.cgColor.alpha > 0)
                #expect(tokens.h6.cgColor.alpha > 0)
                #expect(tokens.bold.cgColor.alpha > 0)
                #expect(tokens.italic.cgColor.alpha > 0)
                #expect(tokens.boldItalic.cgColor.alpha > 0)
                ...
                #expect(tokens.callout.cgColor.alpha > 0)
                #expect(!tokens.h1.isEqual(tokens.h2))
                #expect(!tokens.bold.isEqual(tokens.body))
    ...
    @Test func swappingATokenRecolorsTheMatchingPreviewRange() throws {
        ...
        #expect(originalHeading?.isEqual(ThemeTokens.default.h1) == true)
        #expect((storage.attribute(.foregroundColor, at: bodyRange.location, effectiveRange: nil) as? PlatformColorType)?
            .isEqual(ThemeTokens.default.body) == true)

        var tokens = ThemeTokens.default
        #if os(macOS)
        tokens.h1 = NSColor(red: 1, green: 0, blue: 0, alpha: 1)
        tokens.link = NSColor(red: 0, green: 1, blue: 0, alpha: 1)
        #else
        tokens.h1 = UIColor(red: 1, green: 0, blue: 0, alpha: 1)
        tokens.link = UIColor(red: 0, green: 1, blue: 0, alpha: 1)
        #endif
        #expect(!tokens.h1.isEqual(ThemeTokens.default.h1))

        view.setTheme(tokens)

        #expect((storage.attribute(.foregroundColor, at: headingRange.location, effectiveRange: nil) as? PlatformColorType)?
            .isEqual(tokens.h1) == true)
